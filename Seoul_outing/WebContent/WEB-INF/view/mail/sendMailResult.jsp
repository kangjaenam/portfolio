...
<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<%
	//Controller에서 model.addAttribute("res", Stirng.valueOf(res)); 로 부터 저장된 값 불러오기
	//CmmUtril.nvl() 함수를 통해 model.addAttribute("res") 저장한 값이 NULL인 경우, 0으로 값이 저장되도록 로직 처리함
	//0이면 메일 발송 실패하는 것으로 스프링의 Service에서 정의했기 때문에 0으로 한것임
	String jspRes = CmmUtil.nvl((String)request.getAttribute("res"), "0");
	
	// 웹 URL로부터 전달받는 값들(스프링은 기본적으로 포워드 방식으로 페이지를 이동하기 때문에 url에 입력받은 request 값을 가져올 수 있음, 일반적인 jsp에선 불가능함)
