...
<body background="/images/login.jpg">
	...
	<div class="review_detail_all">
		<form action="/user/modifyReview.do" method="post">
			<!-- 리뷰 제목 날짜 -->
			<h3>
				<input type="text" class="review_write_title" name="title"
					placeholder="제목을 입력해주세요." value='<%=nDTO.getTitle()%>' required="required">
			</h3>
			<small><%=nowtime%></small>
		...
		<%} %>
			...
			<div class="review_contents">
				<textarea name="content" class="review_write_contents"
					placeholder="여기에 내용을 입력해주세요." required="required"><%=nDTO.getContent()%></textarea>
			</div>
			<hr style="margin-top: 20px;" class="reviewlist_hr">
			<button class="reviewlist_button" type="submit">
				완료 <i class="fas fa-edit"></i>
			</button>
			<a href="javascript:window.history.back();"><button type="button"
