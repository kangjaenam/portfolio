...
public class MailController {
	...
	//비지니스 로직(중요 로직을 수행하기 위해 사용되는 서비스를 메모리에 적재(싱글톤패턴 적용됨) 메일 발송을 위한 로직을 구현)
	...
	public String sendMail(HttpServletRequest request, HttpServletResponse response, ModelMap model) throws Exception {
		...
		// 웹에서 받은 값을 DTO에 넣기
		mDTO.settoMail(toMail); // 받는 사람을 DTO 저장
		mDTO.setTitle(title); // 제목을 DTO 저장
		mDTO.setContents(contents); // 내용을 DTO 저장
